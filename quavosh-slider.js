...
function make_dots(n, box, id) {
    var i;
    for (i = 0; i < n; i++) {
        box.innerHTML += '<span class="dot" onclick="currentSlide(' + (i + 1) + ',' + id + ')"></span>';
    }
}
...
function quavosh_slider(qs) {
    ...
    for (y = 0; y < number_of_sliders; y++) {
        ...
        sliders[y].addEventListener('touchend', qs_get_swipe);
...
function S_Carousel(n, id) {
    ...
    if (n > slide_l[id]) { slideIndex[id] = 1 }
...
function S_Rotate(n, id) {
    ...
    var current = (slide_l[id] + slideIndex[id] - 1) % slide_l[id];
    sliders[id].querySelectorAll('.Slides')[current].style.opacity = "100%";
    sliders[id].querySelectorAll('.Slides')[current].style.transform = "rotate(360deg)";
    dots[current].className += " active";
}
...
function S_Scaler(n, id) {
    ...
    var current = (slide_l[id] + slideIndex[id] - 1) % slide_l[id];
    sliders[id].querySelectorAll('.Slides')[current].style.opacity = "100%";
    sliders[id].querySelectorAll('.Slides')[current].style.transform = "scale(1.0)";
    dots[current].className += " active";
}